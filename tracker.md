...
six things (**D14–D19**). **All five frontend ones were fixed and browser-confirmed on 2026-08-07**
(D17 was deliberately left — it needs a product decision and originates in the backend). That
session found **three more** by looking at output: **D19a**, a one-day axis shift the D19 fix itself
introduced; **D21**, money breaking mid-digit in the detail tiles; and **D22**, a test that fails
about half the time.
...
Phases 1–9 are done, browser-verified, and **the five frontend browser findings are fixed and
confirmed at the rendered-pixel level** (2026-08-07). **Phase 10 is unblocked** — Podman 5.8.5 is
installed; its machine is stopped, so `podman machine start` first.

Two things are now genuinely next, and they are independent of each other, so they can run in
parallel terminals.

**Next — `container-podman`, Phase 10.** Nothing blocks it any more. Paste:
...
**Also available, independent:** the optional backend cleanup below (**D10**, **D11**).
...
- ~~**Phase 10** (`container-podman`) needs Podman installed.~~ **Unblocked 2026-08-07** — Podman
  5.8.5 is present, machine stopped (`podman machine start` first). Still true and easy to get
  wrong: `Containerfile.web`'s Node base image must be **22.22.3 or newer** — Angular 22's CLI
  hard-refuses anything older, so a plain `node:22-alpine` tag is only safe if it currently
  resolves above that. Pin it explicitly.
