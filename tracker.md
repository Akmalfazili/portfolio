...
down to **one genuinely open row — D6** — plus one small, already-scoped follow-up: D34's frontend
half (see below). D32/D33/D34 were all closed 2026-08-09, in the same session that also closed
D28/D29 that morning. D6 needs a real NYSE trading day, which no local session can manufacture —
Phase 11 is otherwise the only thing left, and four of its six checks are already reachable today.
...
Also pick up D34's remaining half while you're in the frontend anyway (small,
frontend-angular, not really Phase 11 scope but cheap to fold in): AssetDto now
carries a new `providerHasEverSucceeded` field (true when this asset's
QuoteProviderKind has ever recorded a successful refresh ANYWHERE in this
database, per SourceRefreshStates — not just for this one asset). Read the full
D34 row in the drawback register first. asset-management.page.ts's
`unpricedState()` needs a third suppression condition alongside the existing
`hasEverBeenPriced`/`!isActive` checks: also return null (no warning at all) when
`!asset.providerHasEverSucceeded`, regardless of how many days old the asset is.
This is what stops the escalated "check this identifier" warning firing on a
freshly created database where nothing has ever priced anything yet — measured
live on the container DB on 2026-08-09, where every seeded asset's static
CreatedAt seed value made it look like a 14-day-old identifier problem on day
one. Update or add to the existing D27 frontend test suite; the weekend-threshold
tests should be unaffected since they already run in a fixture where the
