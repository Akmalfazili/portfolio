...
contaminated — see D6's row). Open rows: **D6, D36, D39**.
...
Phases 1–10 and **12** are done and verified; only Phase 11 remains. **D6 is no longer unreachable** —
2026-08-21 proved the stack can be run during real NYSE hours, so the blocker was never the market
calendar, only that nobody had run it then. That session's own figure is not usable (see below), but
the method now exists. Open register rows: **D6, D36, D39**.

> 🔴 **2026-08-21 changed the register again — do not trust any older summary.** The user
> reported stale, inconsistent prices on `/stocks`; the cause was **D38 (new)**, and the finding that
> matters is that **`/stocks` had never once shown a live price in this project's history**. Both
> **D38** and **D37** are now fixed and verified live during real NYSE hours (commit `9ca5828`), and
> all 21 stocks read `priceSource: Live`. **D37's old "benign and self-correcting" verdict was
> measured false** before it was fixed — see its row. **Open rows: D6, D36, D39.** **D39 is new and
> is the one to read first**: the credit ledger added by the D38 fix under-counted real spend by 200
> credits on its first day, and it is the mechanism meant to prevent exactly the overspend D38 caused.

```
Read tracker.md, then the D39 row and the 2026-08-21 handoff entry BEFORE anything
else. D38 and D37 were fixed and verified live on 2026-08-21; do not redo them.
Open rows are D6, D36 and D39.

START WITH D39 — it is the highest-value item and it is a safety mechanism that is
currently wrong. The credit ledger added by the D38 fix reported
`creditsUsedToday: 123` while Twelve Data's own `GET /api_usage` reported
`daily_usage: 323` on the same instant. ~116 of that gap is benign (the table was
created mid-day and starts at zero), but ~70 is not. Do these in order:
  1. MEASURE the per-call cost model before changing it. Issue one `/time_series`
     of a known size and read the `daily_usage` delta; repeat for a larger range.
     The suspicion is that Twelve Data prices that endpoint by output size while
     the ledger charges a flat 1 credit — confirm or kill that theory with a
     number, do not assume it.
  2. Seed the ledger from the real counter on the first write of each UTC day,
     rather than from zero.
  3. Reconcile periodically. NOT per call — `GET /api_usage` costs 1 credit
     itself, so credit monitoring cannot be a tight loop. That fact was measured
     on 2026-08-21 and is easy to rediscover expensively.

D6 is now genuinely reachable and is the other thing worth doing, but ONLY on a
real NYSE trading day with the stack up across a full session. 2026-08-21 was the
project's first-ever live window and it is NOT a valid measurement: the fix landed
mid-session, so the day's total mixes the broken behaviour (~21 credits burned per
failed cycle), the verification spend, and the fixed behaviour. Steady state
projects to ~650/day at 21 symbols, but that is arithmetic again — the whole point
of D6 is to replace arithmetic with a measured figure. Use `GET /api_usage` for it,
and report the number against 800.
