...
| 11 | End-to-end verification | — | 🟨 **Five of six boxes ticked (2026-08-25, second session, all browser-verified).** Only the D6 credit-budget box remains, and it is a measurement awaiting an NYSE close, not a defect |

**Currently active:** none — Phases 1–10 and **12** are closed out. Phase 11 is **five-sixths done**:
its four reachable checks were all browser-verified on 2026-08-25 (second session), the container-DB
box closed with Phase 10, and only the D6 credit box is left. **D6 is no longer blocked**: 2026-08-21
ran the stack through a real NYSE session for the first time, so a credit measurement is now simply a
matter of doing it on a trading day (that session's own number is contaminated — see D6's row).
**D36 is now fully closed, browser gaps included (2026-08-25).** Open rows: **D6 only** — and D6 is
not a code defect, it is a measurement that needs a trading day to happen on.

> 🛑 **2026-08-24 (second session) — the one thing to read before touching Phase 11: A HEALTHY
> RUNNING STACK PROVES NOTHING ABOUT WHICH CODE IT HOLDS.** The stack was found up and green and
> taken at face value; `portfolio-api` predated its own D36 fix by 4½ hours and `portfolio-web` was
> **15 days stale**. The first zero-price POST was rejected, which read exactly like the D36 fix
> having failed — it had simply never been deployed. What identified it was **behaviour, not
> timestamps**: the API returned the *old* validation string while the source contained the new one.
> Rebuilding closed **D36's backend half against the running API** (details in D36's row); the
> manual-DB-insert workaround is retired. **D36's remaining gaps are browser-only.** Also settled at
> the data level, though its Phase 11 box stays unticked because nothing was seen on screen: **Z74
> converts correctly** — `4.42 SGD ÷ 1.26956` (the stored rate for 2026-08-23) `= 3.4815211569 USD`,
> exactly what the API returns, with `255 × 3.4815211569 = 887.7879` matching market value. Since
> CLAUDE.md's contract is that the frontend does no FX math, the *conversion* is proven and only the
> *rendering* is outstanding. ⚠️ **The Chrome extension never connected across the whole session** —
> see the 🔌 note in ▶ Next session before planning any browser work. Credits untouched at **50/800**.

> ⏱️ **2026-08-25 — D6 MEASUREMENT IN PROGRESS. Baseline taken; end reading still outstanding.**
> Today is a Tuesday and a real NYSE trading day, so D6 is finally being measured rather than
> projected. Both pre-flight gates were run first. **Gate A (stale containers) PASSED, and passed on
> behaviour rather than timestamps** as the 2026-08-24 note demands: `portfolio-api` returns the
> **new** D36 string (`"Price per unit cannot be negative."`) to a negative-price POST, and the
> served `main-AUO5L5D3.js` contains the `Queued` fix string
> (`"A larger refresh is already running in the background"`). Image build times (09:21 UTC) also
> postdate HEAD (`b9e6a29`, 08:16 UTC), but that was treated as corroboration, not proof.
> **Baseline, 06:11:51 UTC: Twelve Data `daily_usage` = 24 / 800**, `plan_limit` 8,
> `plan_category` "basic". Cross-checked against `GET /api/prices/status` at 06:09:23 UTC, which
> read `creditsUsedToday: 23`. **Divergence 1 credit — and that 1 credit is the `/api_usage` call
> itself. D39 is confirmed NOT regressed** (its own measured residual was 3; a regression reads
> near 200). Stack brought up and left running; NYSE opens 13:30 UTC and closes 20:00 UTC.
> ⚠️ **Do NOT tick D6 on this baseline** — the row's own text forbids exactly that. The end
> reading at ~20:00 UTC is what closes it.
>
> 📝 **Found while doing the above, unrelated to D6: `CLAUDE.md`'s Market data table is STALE and
> wrong in the same way the "8 req/min" line was.** It states Twelve Data covers
> "US equities, SGX `Z74:XSES`, USD/SGD FX". **Z74 does not go to Twelve Data and cannot** —
> `QuoteProviderKind.Yahoo` exists precisely because Twelve Data's free tier 404s on Z74 with
> "available starting with the Pro or Venture plan" (see the class comment on `YahooQuoteProvider`).
> `tracker.md` knows this (25 mentions of Yahoo); `CLAUDE.md` does not mention Yahoo **at all**.
> This matters for credit arithmetic — Z74 costs **zero** Twelve Data credits — and this file has
> already recorded that a wrong line in `CLAUDE.md` was "the origin of the design error" behind D38.
> ~~Left unfixed pending the user's call, but it should not be left stale.~~ ✅ **FIXED in `c663ee6`**
> — `CLAUDE.md`'s Market data table now routes Z74 to Yahoo and documents why Twelve Data cannot
> serve it. Nothing outstanding here; this note is kept only for the reasoning trail.


> ✅ **2026-08-25 (second session) — PHASE 11'S FOUR BROWSER CHECKS AND ALL THREE D36 BROWSER GAPS**
> **ARE CLOSED. The Chrome extension connected on the FIRST probe.** After two sessions lost to it,
> `tabs_context_mcp` returned a tab immediately with no intervention — no terminals closed, no
> `/chrome` run, nothing done differently. Whatever the cause was, **it was never contention and it
> is not reproducible on demand**; do not spend another session theorising about it, and do not
> assume it will fail next time either. Both pre-flight gates were run first: **Gate A passed on
> evidence, not vibes** — images built 2026-08-24 09:21 UTC, and `git log --name-only` confirms every
> commit since (`c663ee6`, `28e86c5`) touched **only `tracker.md` and `CLAUDE.md`**, so no `src/`
> change was undeployed. Details are in the Phase 11 checkboxes and the D36 row; the handoff log
> entry has the full account.
>
> 🎯 **The session's real find: D36's third gap was never unreachable, and this file was wrong about**
> **it.** The D36 row and the ▶ Next session prompt both stated the blank/na percentage "is NOT
> reachable by adding free shares to an existing position — it needs a holding whose ENTIRE cost
> basis is zero. All 25 assets already hold priced positions… a probe asset would permanently
> pollute the portfolio." **Two such holdings already existed and had for months.** Transaction 2037
> (ANVL, 36,810.905 units) and transaction 2038 (ETH, 0.0007714 units) are both zero-price buys, so
> both assets carry a cost basis of exactly `$0.00`, and the API already returned
> `unrealizedPnlPercent: null` for each. On screen the crypto Holdings table shows ETH `+$1.93` and
> ANVL `+$33.84` with **no percentage at all** — not `Infinity`, not `-100%`, not `NaN` — against AMP
> in the same table showing `-$2,208.57 · -99.14%` as a working control. **No probe asset was needed
> and none was created.** The lesson matches this file's own recurring one: the claim was derived
> from the *assets* list without checking the *transactions* that fed it. Before declaring a state
> unreachable, query for it.
>
> 💵 **Zero Twelve Data credits were spent across the entire session** — ledger read `23/800` at the
> start and `23/800` at the end. Every check was routed through free paths on purpose (CoinGecko
> 2-min crypto cadence, Yahoo 5-min Z74, and `GET /api/prices/status`, which is deliberately built
> never to cost a credit). The manual refresh button and backfill were **not** touched. **D6's
> in-flight measurement is therefore uncontaminated by this session.**

⚠️ **The register is no longer down to one open row.** 2026-08-10 added **D36** (zero-price
acquisitions are unrecordable — worked around by a direct DB insert, not fixed) and **D37** (no
rate-limit pacing, so one backfill run cannot cover every asset), alongside **D35** which was found
and fixed the same day. ~~Open rows are now **D6, D36, D37**. None blocks Phase 11.~~

> 🔑 **2026-08-24 — ACTION REQUIRED OF THE USER, unrelated to any code defect: rotate the Twelve**
> **Data API key.** While preparing D39's measurement, the `backend-dotnet` agent listed
> `dotnet user-secrets` and its redaction regex failed to match PowerShell's `Key = Value` spacing
> (extra spaces around `=`), so the **real key was printed once into that session's transcript**. It
> was **not** written into any project file, **not** logged from application code, and **not**
> committed — the repo is clean — but it is in plaintext in a transcript on disk. Rotate at Twelve
> Data, then update **both** `dotnet user-secrets` under `src/Portfolio.Api` **and** the compose
> `.env`. Until that is done, treat the current key as exposed. **Lesson for any agent doing this
> again: never list user-secrets to obtain a key — read the single value you need and pipe it
> straight into the call, or let the app make the call for you.**
>
> ⚖️ **2026-08-24 — the user has DECIDED NOT to rotate, and this is a closed decision, not an
> outstanding task.** Asked directly, the answer was *"the twelve data api is not rotated. leaving
> it unrotated."* Recorded here so no future session stalls on the banner above or re-litigates it.
> The key remains in plaintext in a transcript on disk, and D6's measurement will spend real credits
> on it. **Do not block work on this and do not raise it again unless the user brings it up** — but
> if the key ever starts 429ing or behaving as though someone else is spending it, this note is the
> first thing to reread.

> ✅ **2026-08-24: D39 is CLOSED — and the theory behind it was killed by measurement, not confirmed.**
> The credit ledger's 200-credit under-count is fixed (`backend-dotnet`, verified live). The
> prescribed first step was to measure the per-call cost model before changing it, and doing so
> **disproved the leading hypothesis**: two direct `/time_series` calls, one returning **5 rows** and
> one returning **1,668 rows** (larger than the 837-row CSCO case that raised the suspicion), cost
> **1 credit each**. Twelve Data does **not** price `/time_series` by output size, and the existing
> flat-1-credit model in `TwelveDataCreditPolicy` was already correct — there was nothing to fix
> there. **Had the fix been written to the theory instead of the measurement, it would have made the
> ledger wrong in the opposite direction.** The real mechanism is structural: the ledger only ever
> counted credits *this process* granted through the throttle, so anything else — the mid-day table
> creation, an out-of-band call, a resilience retry that reaches Twelve Data twice without a second
> `TryAcquireAsync` — drifted it low **permanently**, because it never re-checked itself against
> reality. Fixed by making it self-correcting rather than by chasing each drift source: a new
> `ITwelveDataUsageProvider` seeds a new UTC day's ledger row **from Twelve Data's real counter
> instead of zero**, and reconciles an existing row from that counter once per hour
> (`ReconciliationIntervalMinutes`, in-memory cadence). Reconciliation is deliberately **periodic,
> never per call** — `GET /api_usage` costs 1 credit itself, so credit monitoring cannot be a tight
> loop. `GetStatusAsync` was left untouched on purpose, so polling `/api/prices/status` can never
> cost a credit. The usage provider resolves as nullable, so an unwired caller degrades to the old
> zero-seed rather than throwing. **219/219** (was 213/213 — 6 new tests, **3 confirmed to fail
> against the pre-fix code** by actually reverting `TwelveDataCreditThrottle.cs` and re-running, not
> assumed), build clean. **Verified against the live Docker stack:** before redeploy the ledger read
> 22 against a real 27 (D39's mechanism reproduced in miniature); after redeploy the container log
> recorded `Reconciled Twelve Data credit ledger for 08/24/2026: local 22 -> real 28`, and a
> post-backfill side-by-side read **ledger 50 / real 53**. **Residual 3 credits, not 200** — and that
> residual is the contract, not a leak: 1 credit for the verification read itself plus a few seconds
> of drift since the last ledger write, self-closing at the next reconciliation. ⚠️ **Not verified:**
> that the residual stays small under a full day of sustained load (one seed and one reconciliation
> event were observed live, not a day's worth), and the resilience-retry drift mechanism is named in
> the code comments as *plausible* but was never reproduced directly — periodic reconciliation fixes
> the symptom regardless of which source causes the drift. Open rows: **D6, D36.**

> 🔴 **2026-08-21: read D38 before anything else. The `/stocks` page has NEVER shown a live price.**
> Reported by the user as *"some stocks show the current price as of close at 20 Aug, some at 18 Aug,
> some 7 Aug, some awaiting price"*. Investigation found **three faults stacked, the top one hiding
> the other two**. **D38 (new, High):** `TwelveDataQuoteProvider` batches all 21 symbols into one
> `/quote` call; Twelve Data's per-minute limit is **8 credits** and each symbol costs one, so the
> call spends 21 credits at once and **429s every single cycle** — measured three times over
> (13:32:45, 13:38:01, 13:43:17 UTC) with `symbolsRefreshed: 0` each time, and corroborated by
> `PriceQuotes` holding **zero stock rows, ever**. Every price you have ever seen on `/stocks` is the
> D20 last-close fallback. **D37 (re-classified):** its own "benign and self-correcting" verdict is
> **measured false** — the backfill query has no `OrderBy`, so the same tail is starved every run and
> has advanced **zero days** across two runs; that is what produces the per-asset date spread.
> **D6 (root of the blindness):** the stack had only ever run between SGX close and NYSE open, so the
> market gate meant the broken call was never once allowed to execute. D6's standing note that the
> credit projection was "arithmetic rather than measurement" was right for the wrong reason: the
> arithmetic was against the **daily** budget, and the limit that actually binds is **per-minute**.
> **`CLAUDE.md`'s own line — "800 credits/day and 8 req/min" — is the origin of the design error**
> and has been corrected, or the mistake recurs.
>
> ✅ **Both D38 and D37 were FIXED the same day** (commit `9ca5828`, `backend-dotnet`) and verified
> on the live stack **during real NYSE hours** — a shared credit throttle, ≤8 symbols per request, a
> runtime-derived cadence that widens as the portfolio grows, a persisted credit ledger, and
> least-recently-backfilled ordering. **All 21 stocks now read `priceSource: Live`**, including the
> five that had never held a price in this project's history. The agent also found and fixed a
> further defect during its own live verification: the now-slower manual backfill **504'd through
> nginx**, and the aborted connection's `CancellationToken` cancelled every remaining provider call,
> **each misreported as a genuine provider failure** — now detached onto a background task (HTTP 202).
>
> ⚠️ **Two things did NOT close.** **D39 (new)** — the credit ledger read `creditsUsedToday: 123`
> while Twelve Data's own counter read **323**, so the mechanism meant to prevent overspending
> under-counts by 200 on day one. **D6 stays open** — the fix landed mid-session, so there is still
> no clean full-session credit figure; today's total is contaminated by the broken behaviour plus
> verification spend. Open rows: **D6, D36, D39.**

> 🐛 **2026-08-10: D35 found and fixed — the entire `/stocks` page was returning HTTP 500, and the**
> **cause was one starved provider call.** Reported by the user as *"Couldn't load your holdings"*.
> All three endpoints the page fetches (`/summary`, `/allocation`, `/annual-returns`) were 500ing on
> `InvalidOperationException: No FX rates available to resolve a conversion` — `FxRates` was
> **empty**, and the single SGD asset (Z74) among 20 stocks needs a USD/SGD rate, so **one
> unconvertible asset took down the whole page including the 19 USD holdings that need no FX at
> all.** Two compounding bugs in `PriceBackfillService.RunAsync`: the FX loop ran **last**, sharing
> one `MaxProviderCallsPerRun` budget with the per-asset loop that runs first, so on Twelve Data's
> 8-req/min free tier the one call the page depends on was **guaranteed** to be 429'd with more than
> ~8 stocks held; and `TwelveDataFxProvider.GetHistoryAsync` returned a bare `[]` on failure, so the
> 429 was indistinguishable from "no rates in range" and the run reported **200 OK with
> `fxRatePointsInserted: 0`** and nothing in `assetsFailed`. **Re-running the backfill could never
> have fixed it** — the asset loop exhausts the rate limit before reaching FX, every time. Fixed by
> giving FX first claim on the budget (it is a hard prerequisite; price history is degradable) and
> by adding `FxHistoryFetchResult`, mirroring the `Success`/`Error` split `HistoryFetchResult` has
> had since Phase 4. `dotnet test` **193/193** (was 189/189). **Verified against the live Docker
> stack, not only tests**: `fxRatePointsInserted` 0 → **1721**, all three endpoints 500 → **200**,
> and the page confirmed rendering in a browser. See the D35 row and the handoff entry.
...
> 🛑 **BEFORE BROWSER-VERIFYING ANYTHING, CHECK THAT THE RUNNING CONTAINERS ACTUALLY CONTAIN HEAD.**
> Added 2026-08-24 after this trap was walked into and caught only by luck. A session opened to do
> Phase 11's browser checks and found the stack already up — `docker compose ps` showed all three
> containers healthy, so they were taken at face value. They were **stale**: `portfolio-api` had
> been built at 03:53 UTC and the D36 fix committed at 08:16 UTC, so the API predated its own fix by
> 4½ hours, and `portfolio-web` had been built **2026-08-09 — fifteen days of frontend work missing**,
> including D36's `nonNegativeNumberValidator` and the `Queued` outcome fix. Had the browser
> connected first, the session would have watched both D36 gaps fail and reported *"the D36 fix is
> broken."* It is not broken; it was **not running**. The check is two commands:
>
> ```bash
...
> ```
>
> **The tracker's own "tick only what you have personally seen pass" rule does not protect against
> this** — you can watch something genuinely fail for reasons that have nothing to do with the code
> in the repo, and a failure observed in a browser is exactly the kind of evidence this file treats
> as authoritative. A stale container turns that strength into a liability. Rebuild with
> `docker compose build api web && docker compose up -d` (the `migrate` one-shot re-runs; confirm it
> exits 0) before believing any browser result. Note also that image build time is **not** the same
> as commit time in the other direction: an image built minutes *before* a commit may still contain
> that commit's code if the agent built, verified live, then committed — so use build-time evidence
> to raise suspicion, and confirm against **observed behaviour** (D36 was settled by the API
> returning the old validation string, not by arithmetic on timestamps).

- [x] **Closed 2026-08-25 (second session), in a real browser.** Entered a fractional crypto buy
      through the actual form — ETH, quantity `0.0123456789` (10 dp, accepted with no validation
      error), price `2500`. Persisted exactly: ETH `0.0007714 + 0.0123456789 = 0.0131170789`, cost
      basis `0.0123456789 × 2500 = $30.8642`. The gain/loss card recalculated on screen — cost
      basis `$2,227.83 → $2,258.69`, market value `$55.22 → $85.69`, unrealized `-97.52% →
      -96.21%`, ETH's allocation slice `3.5% → 38%` — every figure matching the API to the cent.
      Probe deleted afterwards and the transaction set diffed back to **byte-identical** with the
      pre-session baseline (136 rows, no added/removed/changed row).
- [x] **Closed 2026-08-25 (second session).** Verified three ways, not one. *Side by side:* stocks
      read cost `$40,617.45` / realized `-$2,073.78` while crypto read `$2,227.83` / `$0.00`;
      neither total contains the other (combined would be `$42,845.28`). *Structurally:* the Stock
      summary contains **0** `assetClass:"Crypto"` rows and the Crypto summary **0** `"Stock"`
      rows; the allocation donuts carry only their own class's symbols. *Causally — the strongest
      of the three:* adding and then removing the crypto probe above moved crypto's cost basis
      `2227.83 → 2258.69 → 2227.83` while stocks stayed at **`40617.4533` throughout**, unchanged
      to the fourth decimal.
- [x] **Closed 2026-08-25 (second session), and the trap in this check was avoided.** A relative
      label drifting "2 min ago" → "3 min ago" is just the clock ticking and proves nothing. What
      was recorded instead, by an in-page sampler on an untouched document: at 07:27:37 UTC the
      toolbar read *"Updated 2 min ago"* and at **07:27:56 UTC it ran BACKWARDS to "Updated just
      now"** — which can only happen if `lastRefreshedAt` actually advanced — then counted up again
      (9s, 24s, 39s, 54s, 1 min). No reload: `performance.getEntriesByType('navigation').length`
      stayed **1** and the sampler's own object survived the whole window. Separately, values
      changed live on the same document: market value `$55.29 → $55.22`, AMP `$19.45 → $19.39`.
      ⚠️ Note for anyone re-running this: the tab was `visibilityState: "hidden"` throughout (D31's
      trap). That does **not** invalidate the result here — `grep` confirms the frontend has **no**
      `visibilitychange`/`visibilityState` handler anywhere, and the timestamp arrives via a
      push-only SignalR hub, not a timer that Chrome would throttle.
- [x] **Closed 2026-08-25 (second session) — both halves, conversion and rendering.** Taken from a
      single simultaneous read inside the page so there is zero time skew between screen and API:
      `currency: "SGD"`, native **4.47 SGD**, USD **3.5181060469**, and the implied rate
      `4.47 / 3.5181060469 = 1.2705699999972335` — **the stored `FxRates` row for 2026-08-24
      (`1.27057`) to the tenth decimal**. Market value `255 × 3.5181060469 = $897.117`, rendered
      `$897.12`. The `/stocks` row displays the **converted USD** price (`$3.52`), never the SGD
      native, which is CLAUDE.md's "the frontend does no FX math" contract holding on screen.
...
| D6 | **The 5/60-minute cadence has never run over a real window** — ⚠️ **measurement STARTED 2026-08-21, the project's first-ever live NYSE window, and it immediately exposed D38** | 5 | Only single cycles and one 2-minute crypto interval have been observed live. The NYSE-open 5-minute cadence, the 60-minute closed cadence and an open→close transition are all unobserved, so "well under 800 credits/day" is still arithmetic rather than measurement. 🔴 **2026-08-21 — the arithmetic was against the wrong limit.** NYSE opened 13:30 UTC with the stack running for the first time in project history, and the very first live stock cycle failed: see **D38**. The **daily** budget was never the binding constraint — the **per-minute credit ceiling (8, not "8 req/min")** is, and one batched `/quote` breaches it on its own. Twelve Data's `GET /api_usage` is the instrument to measure this properly and was not known to this project before today; it returns `current_usage`/`plan_limit` (per-minute) and `daily_usage`/`plan_daily_limit` (daily). **Baseline captured 13:45:16 UTC: `daily_usage: 62 / 800`, `plan_limit: 8`, `plan_category: "basic"`.** Do not tick this row on the baseline alone — it must be a full-session figure taken against the *fixed* D38 behaviour, or it measures a failing call rather than the intended design. | Low, but needs a real trading day — belongs to Phase 11. **Blocked on D38**: measuring credit consumption of a call that 429s every cycle would produce a meaningless number. Sequence is D38 fix → redeploy → measure across the remaining open window. |
...
| D35 | ~~**The FX backfill was starved of its one provider call by the asset loop, and reported success anyway — leaving `FxRates` empty and 500ing every stocks endpoint**~~ **Found and fixed 2026-08-10, reported by the user as the `/stocks` page showing *"Couldn't load your holdings"*.** Two compounding bugs, and the second is what kept the first invisible. **(1) Ordering.** `PriceBackfillService.RunAsync` ran the per-asset price-history loop first and the FX loop **last**, both drawing on one `MaxProviderCallsPerRun` budget (default 20) with no per-minute pacing. Twelve Data's free tier is 8 req/min, so with 17 stocks the asset loop burned the rate limit and the single USD/SGD call at the end was **guaranteed** to 429 — the one call the entire page depends on was structurally always the one that starved. **(2) False success.** `TwelveDataFxProvider.GetHistoryAsync` logged the 429 and returned a bare `[]`, indistinguishable from "the provider genuinely has no rates for this range", so nothing landed in `assetsFailed` and the run reported **200 OK with `fxRatePointsInserted: 0`**. This is the same family as D10/D26/D33 — the reporting layer telling a cleaner story than reality — and notably `HistoryFetchResult`'s own doc comment had called out precisely this bug class for the *quote* path since Phase 4 ("a silent empty list … made a truncated backfill look identical to a transient network failure"); **the FX path simply never got the same treatment.** Fixed by moving the FX loop **before** the asset loop, with `currenciesNeedingFx` derived up front from the already-loaded `earliestTradeDateByAsset` (no extra DB round-trip), and by adding `FxHistoryFetchResult(Points, Success, Error)` with `Ok`/`Failed` factories mirroring `HistoryFetchResult`; `IFxRateProvider.GetHistoryAsync` returns it, and a failed fetch now records `AssetBackfillFailure("FX:USD/SGD", …)`. **The rationale is written into the code, not just here**, because it is the kind of ordering a future refactor would undo as arbitrary: a missing FX rate throws and 500s every USD-reporting endpoint for every holding, USD-denominated or not, while a missing close just leaves a gap in one chart — FX is a hard prerequisite, price history is degradable. **4 new tests** (2 pinning the provider's failure paths, 1 asserting FX claims the budget *before* assets at a budget of 1, 1 asserting a failed FX fetch surfaces in `AssetsFailed`), `dotnet test` **193/193** (was 189/189). **Verified against the live Docker stack, not only unit tests**: `docker compose build api && up -d api` (never `-v`), then `POST /api/prices/backfill` went `fxRatePointsInserted: 0` → **1721** and `assetsProcessed` 9 → **15**; `/api/portfolio/stock/{summary,allocation,annual-returns}` all went **500 → 200**; and the page was **confirmed rendering in a browser** — 17 holdings, `unpricedHoldingsCount: 0`, Z74 converting correctly at 250 units / $397.90 cost / $840.89 market. ⚠️ **Deliberately not fixed here: there is still no rate-limit pacing** — see D37. `backend-dotnet`. | 4, 6 | Fixed the day it was found; the pre-fix payload and root cause are quoted in full above. | — |
| D36 | ~~**A zero-price acquisition cannot be entered — free shares, bonus issues and scrip dividends are unrecordable through the app**~~ **Fixed 2026-08-24, both sides in one session.** `ValidateCommon` now rejects only `pricePerUnit < 0`; the Angular form uses a new `nonNegativeNumberValidator()` while `quantity` stays on the untouched `positiveNumberValidator()`. The row's own advice was followed exactly — the field remains `Validators.required`, so allowing `0` did **not** cost the blank-field typo guard (`Validators.required` counts `0` as present; pinned by a test rather than assumed). The row's claim that only validation stood in the way was **verified rather than inherited**: the `costBasisUsd > 0m` guard at `PortfolioSummaryService.cs:208`, `AverageCostCalculator`'s `quantity == 0m` guard, and the absence of any percentage division in `PortfolioPerformanceService` were all read directly — no divide-by-zero anywhere, so the fix stayed validation-only as predicted. 222/222 backend, 206/206 frontend. ⚠️ **Two honest gaps.** First, the **frontend** verify-by-reverting is weaker evidence than the backend's: reverting the validator source produced a *compile* error (`TS2724`, missing export), which proves the test references the new code but not that it asserts the right behaviour — only the `Queued` half was reverted in a way that reproduced a genuine behavioural failure. Second, **nothing was driven in a browser**: transaction id 2002 (the Z74 free-share row this fix retires) has never been opened in the real edit dialog, so "the UI now accepts what the DB already holds" is a unit-test claim, not a seen-it-work claim. Both are cheap to close in the next session that has a stack up — see Phase 11. ✅ **2026-08-24, later the same day: the backend half is now proven against the RUNNING API, not just unit tests — and proving it is what exposed the stale-container trap now documented under Phase 11.** The first attempt to POST a zero-price transaction to the live stack was **rejected**, which looked like the fix having failed; it was the container predating the fix by 4½ hours. After rebuilding, the same three probes against the real API: negative price → **HTTP 400** carrying the *new* string `"Price per unit cannot be negative."` (so the guard was narrowed, not removed — and the old vs new message is what identified the stale image in the first place); zero price → **HTTP 201 Created**; and the resulting holding behaved exactly as free shares must — Z74 quantity **255 → 258** with cost basis **unchanged at `$397.9006`**, market value `$898.2325` reconciling exactly to `258 × 3.4815211569`. Probe transaction deleted afterwards (**HTTP 204**, then **404** on re-read) and every baseline figure confirmed restored to the decimal, so no probe data survives. **The manual-DB-insert workaround is retired** — a free-share row no longer needs hand-insertion. ⚠️ **Still open, and deliberately not manufactured:** (a) nothing has been driven in a **browser** — transaction 2002 has still never been opened in the real edit dialog, and a zero-price entry has never gone through the *form*, because the Chrome extension would not connect all session; (b) the **blank-percentage path is still only a code read** — rendering `null` rather than `Infinity` requires a holding whose *entire* cost basis is zero, every one of the 25 assets already has a priced position, and the assets API has **no DELETE**, so creating a probe asset would permanently pollute the portfolio. That case was left unverified rather than faked. ✅ **2026-08-25 (second session): ALL THREE remaining gaps closed in a real browser — D36 is now fully done.** (a) **Transaction 2002 opened in the actual edit dialog** for the first time in this project's history: the form loaded `Price per unit: 0` with **zero** `.ng-invalid` nodes anywhere in the dialog and Save enabled. That alone would only prove a *pristine* control passes, so the validator was driven both ways — typing `-1` turned the field red and produced the live error string **`"Cannot be negative."`** (D36's own frontend message, seen on screen for the first time), and returning it to `0` cleared the error on a now-**dirty** control. Saved successfully; the API confirms 2002 unchanged afterwards — quantity 5, price 0, fees 0, SGD, notes `'Free shares - zero cost acquisition'` preserved, Z74 cost basis still `$397.9006`. (b) **A brand-new zero-price transaction was entered through the FORM, end to end** — ETH, quantity `0.005`, price `0` → saved, listed as `2026-08-25 | ETH | Buy | 0.005 | $0.00 | $0.00`, and behaving exactly as free shares must: ETH quantity `0.0131170789 → 0.0181170789` with **cost basis unchanged at `$30.8642`**. The API path was already proven; **the form path now is too**. (c) **The blank-percentage path is closed, and the premise of the gap above was wrong** — see the 🎯 note at the top of this file. It needed no probe asset: ANVL (txn 2037) and ETH (txn 2038) are *already* zero-price acquisitions with `$0.00` cost basis, and the crypto Holdings table renders both with **no percentage at all** beside their `+$33.84` / `+$1.93`, against AMP's `-99.14%` in the same table as a control. Corroborated causally in the same session: the moment the ETH probe gave ETH a real cost basis, its `unrealizedPnlPercent` flipped `null → 6.2801`, and reverted to `null` when the probe was deleted — so the blank is genuinely cost-basis-driven, not a hardcoded special case. **Both probes deleted; the transaction set diffs byte-identical to the pre-session baseline (136 rows, none added, removed or changed).** 📝 One observation, deliberately **not** filed as a defect: the dialog's Save button stays `disabled: false` on an invalid form, but submission is genuinely guarded — clicking it with `-1` in the price field left the dialog open, the error displayed and the record untouched. That is a legitimate Material pattern (guard in the handler rather than disabling the control), and it keeps the error reachable for screen-reader users instead of presenting a dead button; recorded so a future session recognises it as intentional rather than re-investigating it. | 3 | **Found 2026-08-10 when the user tried to record 5 free Z74 shares.** `TransactionService.ValidateCommon` rejects `pricePerUnit <= 0` with *"Price per unit must be positive."* — this is **server-side**, so it is not a frontend-validator workaround; the API refuses it too. Zero-cost acquisitions are a real and common category: broker free-share promotions, bonus/scrip issues, stock grants. **The calculators are already ready for them and only validation is not**: `PortfolioSummaryService.cs:208` deliberately guards `costBasisUsd > 0m` and returns a `null` percentage otherwise, which is only reachable if a zero-cost holding can exist — so the codebase anticipates the case while validation forbids creating it. Note `fees` is correctly `< 0` (zero allowed); only price uses `<= 0`. **Worked around, not fixed**: the user's 5 Z74 shares were inserted directly into `Transactions` (id 2002, 2020-12-25, `Notes: 'Free shares - zero cost acquisition'`), with every other validation rule applied by hand — quantity positive, fees non-negative, date not future, currency matching the asset's SGD. Verified through the API afterwards: quantity 250 → **255**, cost basis **unchanged** at `$397.9006`, market value `$840.89` → `$857.71`, percent `+111.33%` → `+115.56%` — which is exactly correct behaviour for free shares. **Until this is fixed, any future free-share record needs the same manual insert.** | Low — change `pricePerUnit <= 0` to `< 0` in `ValidateCommon` and mirror it in the Angular form's validator, which must be done **together** or the UI will reject what the API accepts. Worth deciding one thing deliberately rather than by default: a blank price field currently fails this check, so allowing 0 removes a real typo guard — consider requiring the field to be *present* while permitting the value 0, rather than dropping the check entirely. `backend-dotnet` + `frontend-angular`. |
| D37 | ~~**The backfill has no rate-limit pacing, so a single run structurally cannot fetch every asset**~~ **FIXED 2026-08-21** (commit `9ca5828`) — least-recently-backfilled ordering (nulls first) so no asset can be starved twice running, the shared credit throttle for pacing, and the run budget derived from remaining daily credits instead of the hardcoded 20. Proven live: a second manual backfill served the previously-failed asset **first**, and the never-once-reached CSCO/KO/PINS/TDOC/VZ finally got history. ⚠️ **This row's original "benign, self-correcting" verdict was MEASURED FALSE before it was fixed — read the correction below, it is the more useful record.** | 4 | 🔴 **2026-08-21 correction, from data rather than argument. Two things this row asserted are wrong.** **(1) It is not self-correcting.** The claim "the fetch is idempotent … so a later run fills the gaps" assumed successive runs serve a different set. They do not: `PriceBackfillService.RunAsync` iterates `db.Assets.Where(…)` (line ~74) with **no `OrderBy`**, so SQL Server returns clustered-index (Id) order on every run — the same head is served and the same tail starved, permanently. Measured after two further runs (08-14, 08-21): the starved assets have advanced **zero days**. Newest `PriceHistory` per asset on 2026-08-21 — 08-20 for AAPL/MSFT/AMZN/AVGO/ERIC/FSLY/HLAL/JNJ, 08-19 ARVLF, 08-21 Z74 (Yahoo, unmetered, never throttled), **08-07 for NIO/NVDA/PG/SPUS/SPWO/TTD/XOM (Ids 1007–1013, created 08-10, backfilled exactly once and never since)**, and **zero rows at all for CSCO/KO/PINS/TDOC/VZ (Ids 2007–2011, never once reached)**. Both starved sets are contiguous Id blocks, which is the ordering fingerprint. The user's report — *"some stocks show the price as of 20 Aug, some 18 Aug, some 7 Aug, some awaiting price"* — is precisely this row, surfaced on the `/stocks` page. **(2) "Credit budget is not the constraint; requests per minute is" is wrong on both halves.** The per-minute ceiling is **credit**-denominated, not request-denominated (see **D38**), and the daily budget *is* a real constraint once a paced design is costed: a full sweep of N symbols every T minutes over a 390-minute NYSE session needs `(390/T)·N` credits plus `N+1` for the backfill, i.e. `T ≥ 390N/(799−N)` — about 11 min at today's N=21, ~56 min at N=100. A third aggravation this row never named: on the 08-14 run the resilience circuit breaker opened after enough 429s, converting a soft rate limit into a hard stop for the remainder of the run (*"The circuit is now open and is not allowing calls."* for TTD/XOM/CSCO/KO/PINS/TDOC). **Original 2026-08-10 text kept below unedited, as the record of what was believed at the time.** — **Split out of D35 on 2026-08-10 rather than left implied.** D35 fixed the *ordering* so the critical FX call is no longer the one that starves, but the underlying mismatch remains: Twelve Data's free tier allows **8 requests/minute**, the backfill makes up to `MaxProviderCallsPerRun` (default 20) in an uninterrupted burst, and the only backstop is a Polly retry of 2 attempts at 2s exponential — roughly **6 seconds against a 60-second window**, which cannot outlast it. Measured on the post-D35 run: 15 of 17 stocks processed, **HLAL and JNJ still 429'd**. Now benign rather than page-breaking — the fetch is idempotent against the `(AssetId, Date)` unique index, so a later run fills the gaps, and nothing 500s — but it means "one backfill run brings history up to date" is false and the register should say so rather than let a future session rediscover it. Credit budget is not the constraint (800/day, comfortably unspent); **requests per minute is.** | Low–medium — a token-bucket or a paced delay in the Twelve Data pipeline sized to 8/min, which makes a full run take minutes rather than seconds and should therefore stay off the request path (the background service, not the manual endpoint, or the endpoint returns before finishing). Alternatively raise nothing and accept multi-run convergence, but then say so in the endpoint's own doc comment. `backend-dotnet`. **⚠️ 2026-08-21: "accept multi-run convergence" is no longer an option — there is no convergence. Any fix must change the ordering (least-recently-backfilled first) as well as add pacing; pacing alone would make the starvation rotate, not end.** |
| D38 | ~~**The live stock quote call can never succeed — one `/quote` batch of every symbol spends more credits than the per-minute ceiling allows, and 429s every cycle**~~ **Found and FIXED 2026-08-21, the same day, and verified on the live stack during real NYSE hours** (commit `9ca5828`): chunking to ≤8 symbols per request behind a shared `ITwelveDataCreditThrottle`, a runtime-derived cadence, and a persisted daily credit ledger. **All 21 stocks now read `priceSource: Live`, `unpricedHoldingsCount: 0`, TwelveData `symbolsRefreshed: 21` — including CSCO/KO/PINS/TDOC/VZ, which had never held a price in this project's history.** ⚠️ One residual left open: **D39** (the new ledger under-counts real spend). Original finding kept in full below. | 5 | 🔴 **Found 2026-08-21, reported by the user as stale and inconsistent prices on `/stocks`. This is the direct cause: `/stocks` has never once displayed a live price, on any day, since the portfolio grew past 8 Twelve Data symbols on 2026-08-09/10.** `TwelveDataQuoteProvider.GetQuotesAsync` (lines 49–50) joins every routed symbol into a single `quote?symbol=A,B,C,…` request — today 21 symbols. **Twelve Data's free-tier per-minute limit is 8 CREDITS, and each symbol in a batch costs one credit**, so that one request spends 21 credits instantly and is rejected. **Measured across three consecutive open-market cycles** (NYSE opened 13:30 UTC; cycles at **13:32:45**, **13:38:01** and **13:43:17**, each with a clean rate-limit window between them and the prior backfill 20+ min earlier): every one returned `symbolsRefreshed: 0`, `lastRunSuccess: false`, `lastError: "Twelve Data returned HTTP 429."`. Corroborated in the database — `PriceQuotes` holds exactly **3 rows, all crypto** (ETH/AMP/ANVL); **zero stock quotes have ever been written**, so `PortfolioSummaryService` can never reach its `PriceSource.Live` branch for a stock and always falls through to the D20 last-close fallback. That is why every row reads `Close · <date>` with per-asset dates: the dates are D37's starvation, and the *absence of any live price at all* is this row. **The denomination is confirmed, not inferred**: Twelve Data's own `GET /api_usage` returns `plan_limit: 8` (the per-minute credit counter) alongside a separate `plan_daily_limit: 800`. It is also deducible from the behaviour alone — a single request carrying 21 symbols 429s, so the per-minute limit cannot be counting *requests*, as that request is #1 in its window. **Root cause of the design error is a line in `CLAUDE.md`**: "Twelve Data free tier is 800 credits/day and **8 req/min**". Batching hard is the correct defence against the daily budget and the exact opposite of what a credit-denominated per-minute ceiling wants, so the provider was written to optimise one limit while structurally violating the other. The class comment at `TwelveDataQuoteProvider.cs:13-15` reasons explicitly about the 800/day budget and never mentions the per-minute one. **Why it hid for 11 days: the market-open gate.** `PriceRefreshService` never spends a credit on a closed market, and the stack has only ever been run between SGX close (09:00 UTC) and NYSE open (13:30 UTC) — see the `RefreshRuns` windows on 08-10 (11:16–11:28), 08-14 (12:12–12:20) and 08-21 (13:11+). The broken call was never once allowed to execute until today. **This is what D6 was standing in front of**: that row called the credit projection "arithmetic rather than measurement", but the arithmetic was against the wrong limit — the daily budget was never the binding constraint, and the first time the code was permitted to make a live stock call it failed. 🔴🔴 **WORSE, AND MEASURED: the rejected call still spends every one of its 21 credits.** Tracked against `GET /api_usage` across two cycles, each isolated by readings with no intervening call (the instrument itself costs 1 credit per call, which accounts for the +1 baseline drift): `daily_usage` **65 → 87** across the 13:48:42 cycle and **91 → 113** across the 13:53:47 cycle — **+22 each time, i.e. +1 poll and +21 for a batch that returned HTTP 429 and zero prices.** So the broken refresh is not merely useless, it is **actively burning ~21 credits every 5 minutes of market open for nothing**: ~72 cycles over a 390-minute session ≈ **1,512 credits/day against a 800/day budget**, exhausting the entire daily allowance after roughly **38 cycles (~3.4 hours)**. **This means D38 also destroys D37's ability to recover**: if the stack is left running across an NYSE close, the daily backfill (which fires once per day while NYSE is closed) finds the budget already blown and 429s every asset, so the starved tail can never catch up. The two open defects are not independent — D38 must be fixed first or D37's fix cannot function. | **High.** Not a pacing tweak: the batching strategy itself is wrong for the limit that actually binds, and any fixed cadence is correct for exactly one portfolio size — which is how this bug was born (fine at 2 symbols, broken at 21). Fix is a **credit-aware throttle** (≤8 credits per rolling 60s, a `/quote` batch costing one credit *per symbol*, chunked into `ceil(N/8)` requests) shared by the quote and backfill paths since they draw on one limit, a **cadence derived at runtime** from the live symbol count and remaining daily budget rather than hardcoded, a **persisted daily credit ledger** so the budget is tracked from real spend rather than assumed — the assumption is exactly what hid this — and both surfaced on `GET /api/prices/status` so freshness degrades *visibly* as the portfolio grows. Must stay off the request path: a paced sweep takes minutes. `backend-dotnet`. |
| D39 | ~~**The new daily credit ledger under-counts real Twelve Data spend — the safety mechanism that prevents D38-style overspending is itself miscalibrated**~~ **Fixed 2026-08-24 — and the fix's own premise was disproved by the measurement it was told to take first.** The row below suspected `/time_series` was priced by output size while the ledger charged a flat 1 credit. **Measured: it is not.** A 5-row call and a **1,668-row** call each cost exactly **1 credit** — so the flat-1 model in `TwelveDataCreditPolicy` was already right, and writing the fix to the theory would have skewed the ledger the *other* way. The actual mechanism is structural rather than arithmetic: the ledger counted only what *this process* granted through the throttle, so the mid-day table creation, any out-of-band call, or a resilience retry that reaches Twelve Data without a second `TryAcquireAsync` drifted it low **permanently** — it never re-checked itself against reality. Fixed by making it self-correcting instead of enumerating drift sources: new `ITwelveDataUsageProvider` + `TwelveDataUsageProvider` (no retry policy — retrying a 1-credit monitoring call spends the budget it measures) seeds a new UTC day's row **from the real counter, not zero**, and reconciles an existing row hourly (`ReconciliationIntervalMinutes`, in-memory, matching the existing rolling window's lifecycle — no migration, no schema change). Periodic **not** per call: `GET /api_usage` costs 1 credit itself. `GetStatusAsync` deliberately untouched, so polling `/api/prices/status` can never cost a credit; the provider resolves nullable so an unwired caller degrades to the old behaviour rather than throwing. **219/219** (was 213/213; 6 new tests, **3 confirmed to fail against the pre-fix code by reverting and re-running**), build clean. **Live-verified on the running stack**: ledger 22 vs real 27 before redeploy, then `Reconciled Twelve Data credit ledger for 08/24/2026: local 22 -> real 28` in the container log, and **ledger 50 / real 53** side by side after a real backfill — **residual 3, not 200**, which is the design's accepted contract (the verification read's own credit plus seconds of drift), self-closing at the next reconciliation. ⚠️ **Not verified:** the residual's behaviour across a *full day* of sustained load (one seed + one reconciliation event observed, not a day's worth), and the retry-drift mechanism is flagged in code comments as plausible but was never directly reproduced — reconciliation fixes the symptom whatever the source. Kept in full below. | 5 | **Found 2026-08-21 by the orchestrating terminal while verifying D38's fix, by cross-checking the ledger against Twelve Data's own counter rather than trusting it.** Measured at 14:48 UTC: `GET /api/prices/status` reported **`creditsUsedToday: 123 / 800`** while Twelve Data's `GET /api_usage` reported **`daily_usage: 323 / 800`** — a **200-credit divergence**, i.e. the ledger believed 677 credits remained when 477 actually did. Part of the gap is benign and one-off: the ledger table was created mid-day and starts at zero, so ~116 credits already spent before deployment are invisible to it. **The remaining ~70 are not explained by deployment timing** (my own `/api_usage` polls account for only ~13). **Probable mechanism, not yet measured**: the ledger charges `/time_series` a flat 1 credit, but Twelve Data prices that endpoint by output size, and the verification runs pulled multi-year ranges — 837 rows for CSCO alone. This matters because the ledger is precisely the component meant to stop the class of overspend D38 caused: a ledger that under-counts will authorise calls the real budget cannot afford, and those calls 429 while still being charged (see D38's own measurement that a rejected batch is billed in full). Note the fixing agent flagged non-reconciliation as a known limitation and framed it as a nice-to-have follow-up; the measurement shows it is material on day one. | Low–medium. Reconcile the ledger against `GET /api_usage` periodically (**not** per call — that endpoint costs 1 credit itself, so credit monitoring cannot be a tight loop), seed it from the real counter on first write of a day rather than from zero, and correct the per-call cost model for `/time_series` by output size. Worth one measured experiment first: issue a single known-size `/time_series` and read the delta, rather than assuming the pricing rule. `backend-dotnet`. |
...
| 2026-08-10 | `backend-dotnet` | D35 (new), D36 (new, worked around), D37 (new, open) | **A user-reported page failure traced to a structurally-guaranteed provider starvation, fixed and confirmed on the live stack.** Symptom: `/stocks` showing *"Couldn't load your holdings"*. Diagnosis was done from the orchestrating terminal before any agent ran — all three endpoints 500ing on `No FX rates available to resolve a conversion`, `SELECT COUNT(*) FROM FxRates` returning **0**, and one SGD asset (Z74) among 20 stocks being enough to take the entire page down, **including the 19 USD holdings that need no FX at all**, because the exception escapes `BuildHoldingsAsync` rather than degrading that one row. **The instructive part is why the obvious remedy would not have worked:** running `POST /api/prices/backfill` returned a clean `200` with `fxRatePointsInserted: 0`, and re-running it could never have helped — the per-asset loop ran first and burned Twelve Data's 8-req/min limit before the single FX call at the end, every time, so the one call the page depends on was always the one that 429'd. That was only visible because the API log line (`Twelve Data /time_series (FX) for USD/SGD failed with status 429`) contradicted the endpoint's own success payload; the provider returned a bare `[]` on failure, so `assetsFailed` was empty and the run looked fine — **the same false-success family as D10/D26/D33, and `HistoryFetchResult`'s doc comment had warned about exactly this bug class for the quote path since Phase 4 while the FX path never got the treatment.** Delegated to `backend-dotnet` per CLAUDE.md with the diagnosis supplied so the agent did not re-derive it; it reordered the loops, added `FxHistoryFetchResult`, and reported **193/193** (was 189/189) with 4 new tests. **Verified independently of the agent, and the verification is what makes this closable**: reviewed the full diff (the reordering is faithful and `currenciesNeedingFx` is correctly derived up front from the already-loaded dictionary, no extra query), rebuilt and redeployed `api` (never `-v`), then measured `fxRatePointsInserted` **0 → 1721**, `assetsProcessed` **9 → 15**, all three endpoints **500 → 200**, and drove Chrome against `http://localhost:8080/stocks` — the page renders, 17 holdings, `unpricedHoldingsCount: 0`, allocation pie and annual-return chart both drawing. A useful side effect worth recording: putting FX first also **improved** price-history coverage 9 → 15 assets, because the FX prerequisite is one request while the tail it used to sit behind is many. 📝 **Also handled, and recorded as D36 rather than silently patched:** the user could not enter 5 free Z74 shares because `ValidateCommon` rejects `pricePerUnit <= 0` **server-side** — not merely a frontend validator, so there was no UI workaround. Inserted directly into `Transactions` (id 2002) with every other validation rule applied by hand, and verified through the API that the result is correct: quantity 250 → 255, cost basis **unchanged** at `$397.9006`, market value `$840.89` → `$857.71`. The validation itself is left open deliberately — it needs a paired backend+frontend change and one real decision (allowing `0` removes a blank-field typo guard), which is not this session's to make unilaterally. ⚠️ **Left knowingly: D37**, no rate-limit pacing — HLAL and JNJ still 429'd on the post-fix run. Now benign (idempotent, converges over runs, nothing 500s) but it means "one backfill run brings history up to date" is false. **Twelve Data credits**: ~36 spent across two backfill runs against 800/day; the binding constraint is requests-per-minute, not the daily budget. |
| 2026-08-21 | `backend-dotnet` + orchestrating terminal | **D38 (new, High)**, D37 (re-classified and fixed), D6 (measurement started) | **The user reported inconsistent stale prices on `/stocks` — *"some show close at 20 Aug, some 18 Aug, some 7 Aug, some awaiting price"* — and the investigation found three faults stacked, the top one hiding the other two. This session also contains the project's first-ever live NYSE trading window.** 🔴 **D38, new and the direct cause: `/stocks` had NEVER displayed a live price, on any day.** `TwelveDataQuoteProvider` batched all 21 symbols into one `/quote` call; Twelve Data's per-minute ceiling is **8 credits** (not "8 req/min") and each symbol costs one, so the call spent 21 credits at once and 429'd. **Measured three times over** at 13:32:45, 13:38:01 and 13:43:17 UTC, each `symbolsRefreshed: 0`, each with a clean window between them — and corroborated in the database: `PriceQuotes` held exactly 3 rows, all crypto, **zero stock quotes ever written**. The denomination was then confirmed outright rather than inferred, via an instrument this project did not previously know it had: Twelve Data's `GET /api_usage` returns `plan_limit: 8` (per-minute credits) alongside a separate `plan_daily_limit: 800`. 🔴🔴 **And the rejected call still spent all 21 credits** — `daily_usage` **65 → 87** and **91 → 113** across two isolated cycles (+22 each: +1 for the instrument's own call, +21 for a batch that returned nothing), i.e. ~1,512 credits/day projected against a 800 budget, exhausting the allowance in ~3.4 hours and thereby **starving the backfill too** — the two defects were not independent. **D37 was re-classified, not merely fixed: its own "benign and self-correcting" verdict was measured false.** `PriceBackfillService` iterated `db.Assets.Where(…)` with **no `OrderBy`**, so SQL Server returned clustered-index order every run and the same tail starved permanently — across two runs (08-14, 08-21) the starved assets advanced **zero days**, and both starved sets were contiguous Id blocks (1007–1013 stuck at 08-07; 2007–2011 never reached at all), which is the ordering fingerprint. **D6 is the root of the blindness**: the stack had only ever run between SGX close (09:00 UTC) and NYSE open (13:30 UTC) — see the `RefreshRuns` windows on 08-10, 08-14 and 08-21 — so the market gate meant the broken call was never once permitted to execute. D6's standing note that the credit projection was "arithmetic rather than measurement" was right for the wrong reason: the arithmetic was against the **daily** budget, and the limit that binds is **per-minute**. **`CLAUDE.md`'s own line — "800 credits/day and 8 req/min" — was the origin of the design error** (batching hard is correct for the daily budget and exactly wrong for a credit-denominated per-minute ceiling) and has been corrected. **Fix, by `backend-dotnet`** (commit `9ca5828`): a shared `ITwelveDataCreditThrottle` (rolling 60s/8-credit window, one lock across both the quote and backfill paths so they cannot jointly overspend), chunking to ≤8 symbols per `/quote`, a **runtime-derived cadence** (`TwelveDataCadenceCalculator`, `T ≥ sessionMinutes·N/(budget−N−1)`, floored at 5 min) so the interval widens automatically as the portfolio grows rather than silently breaking as it did at 21 symbols, a persisted daily credit ledger (new table + migration `20260821141246_AddTwelveDataCreditLedger`), least-recently-backfilled ordering for D37, and `effectiveTwelveDataIntervalSeconds`/`creditsUsedToday`/`creditBudget` added to `GET /api/prices/status`. The agent independently re-derived the 193/193 baseline in a clean worktree at `HEAD` rather than trusting this file's number, and confirmed its two key tests fail against the pre-fix code by actually reverting and re-running. **213/213**, build clean. 🐛 **The agent found and fixed one further defect during its own live verification**: once every call is throttled, a full manual backfill takes minutes, so `POST /api/prices/backfill` **504'd through nginx** at the default 60s proxy timeout — and the aborted connection's `CancellationToken` then cancelled every remaining provider call, **each one misreported as a genuine provider failure** in the audit trail. Fixed by detaching onto a background task (HTTP 202), matching the same treatment given to `/prices/refresh`. **Verified independently of the agent, against the live stack during real NYSE hours**: all 21 stocks now read `priceSource: Live` at today's session — **including CSCO, KO, PINS, TDOC and VZ, which had never held a price in this project's history** — `unpricedHoldingsCount: 0`, TwelveData `symbolsRefreshed: 21` with `lastError: null`, derived cadence 780s. ⚠️ **One residual found by that verification and left open as D39**: the new ledger read `creditsUsedToday: 123` while Twelve Data's own `daily_usage` read **323** — a 200-credit divergence in the very mechanism meant to prevent overspending. Partly benign (the ledger starts at zero mid-day, after ~116 was already spent), but ~70 credits remain unexplained after subtracting the instrument's own polls. **Also learned: `GET /api_usage` itself costs 1 credit per call**, so credit monitoring cannot be a tight loop. **D6 is NOT ticked** — the fix landed mid-session, so no clean full-session figure exists; today's total is contaminated by the broken behaviour plus verification spend and is expected to land near the cap, while steady state from tomorrow projects to ~650/day. |
| 2026-08-24 | `backend-dotnet` + orchestrating terminal | **D39 (closed)** | **D39 fixed and live-verified, with the headline being that the measurement the task mandated *before* touching code disproved the very theory the fix was going to be built on.** D39's row suspected Twelve Data prices `/time_series` by output size while the ledger charges a flat 1 credit — plausible, since the 2026-08-21 verification had pulled 837 rows for CSCO alone. Two direct calls settled it: `AAPL outputsize=5` (**5 rows**) and `AAPL 2020-01-01→2026-08-24` (**1,668 rows**, twice the size of the case that raised the suspicion) each moved `daily_usage` by the same amount — **1 credit apiece**. The flat-1 model in `TwelveDataCreditPolicy` was **already correct**; had the fix been written to the hypothesis it would have made the ledger wrong in the opposite direction. This is the second time in three sessions that a confidently-stated cost model in this file turned out to be arithmetic nobody had measured (cf. D38's "8 req/min", and D4/D13's stale estimates) — **the measure-first instruction earned its keep and should stay in every future prompt of this kind.** **Real mechanism, structural not arithmetic:** the ledger only ever counted credits *this process* granted through the throttle, so the mid-day table creation, any out-of-band call, or a resilience retry reaching Twelve Data without a matching second `TryAcquireAsync` drifted it low **permanently** — nothing ever re-checked it against reality. Fixed by making the ledger self-correcting rather than by enumerating drift sources: new `ITwelveDataUsageProvider` (Application) + `TwelveDataUsageProvider` (Infrastructure, **deliberately no retry policy** — retrying a 1-credit monitoring call spends the budget it exists to measure), `TryAcquireAsync` now seeds a new UTC day's ledger row **from Twelve Data's real counter instead of zero** and reconciles an existing row once per `ReconciliationIntervalMinutes` (default 60, in-memory, matching the existing rolling window's lifecycle). **No migration and no schema change** — `TwelveDataCreditLedgerEntry` is untouched. Three deliberate design choices worth not re-litigating: reconciliation is **periodic, never per call** (`GET /api_usage` costs 1 credit itself — confirmed again here, every measured delta above includes it); `GetStatusAsync` was left alone so polling `/api/prices/status` can **never** cost a credit; and the usage provider resolves per-scope via nullable `GetService`, so a caller or test that hasn't wired one degrades to the pre-fix zero-seed instead of throwing. **219/219** (was 213/213 — 6 new tests), build clean, **and the 3 seeding/reconciliation tests were confirmed to genuinely fail against the pre-fix code** by stashing `TwelveDataCreditThrottle.cs` back, re-running (expected 305 got 5; expected 401 got 6; usage provider never called at all), then restoring and re-verifying green. **Independently re-verified by the orchestrating terminal rather than taken on the agent's word**: `dotnet test` re-run from this terminal → 212 unit + 7 integration = **219/219**, and `GET /api/prices/status` on the live stack read `creditsUsedToday: 50`, matching the agent's reported figure exactly. **Live verification of the fix itself:** before redeploy the ledger read **22** against a real **27**, reproducing D39's mechanism in miniature; after `docker compose build api && up -d --no-deps api` the container log recorded `Reconciled Twelve Data credit ledger for 08/24/2026: local 22 -> real 28`, and a post-backfill side-by-side read **ledger 50 / real 53** — **residual 3, not 200**, which is the accepted contract (1 credit for the verification read itself plus seconds of drift since the last ledger write) and self-closes at the next reconciliation or process restart. ⚠️ **Explicitly not verified:** the residual under a *full day* of sustained load — one seed and one reconciliation event were observed live, not a day's worth; and the resilience-retry drift mechanism is named in the code comments as *plausible* but was never directly reproduced, since periodic reconciliation corrects the symptom regardless of source. No integration test hits the real Twelve Data API, by design — those cost real credits. 🔑 **One operational incident, not a code defect:** while listing `dotnet user-secrets` to prepare the direct-API experiment, the agent's redaction regex missed PowerShell's `Key = Value` spacing and **the real Twelve Data key was printed once into that session's transcript**. Not written to any file, not logged from application code, not committed — but exposed in plaintext on disk. **The user must rotate the key and update both `dotnet user-secrets` and the compose `.env`;** see the 🔑 banner at the top of this file. Future agents: read the one value you need or let the app make the call — never list the whole secret store. |
| 2026-08-24 | `backend-dotnet` + `frontend-angular` | **D36 (closed)** + frontend handoff | **D36 fixed on both sides in one session, and the `Queued` messaging regression closed with it.** Backend: `ValidateCommon` moved `pricePerUnit` from `<= 0` to `< 0` (*"Price per unit cannot be negative."*); **`quantity` deliberately left at `<= 0`** so the loosening could not leak sideways. Frontend: a new `nonNegativeNumberValidator()` was added *alongside* the untouched `positiveNumberValidator()` — `pricePerUnit` is the only field that swaps, `quantity` keeps rejecting 0. The field stays `Validators.required`, so the blank-field typo guard the D36 row asked to preserve survives: `Validators.required`'s `isEmptyInputValue` checks only null/undefined/length, so `0` counts as present — **confirmed by a dedicated test rather than assumed**. The tracker's standing claim that this was "validation only" was **checked, not inherited**: `PortfolioSummaryService.cs:208` guards `costBasisUsd > 0m` and returns a `null` percentage; `AverageCostCalculator`'s single division is separately guarded by `quantity == 0m`; `PortfolioPerformanceService` has no percentage division at all. No divide-by-zero, no scope expansion. **Frontend handoff closed in the same pass:** `RefreshOutcome` gained `'Queued'`, and `describeRefreshOutcome` now handles it *before* any branch reading `totalSymbolsRefreshed`/`sources` — a queued sweep carries `0` and an empty array, which is byte-identical to "nothing was due", so it rendered **"Already up to date — nothing changed since the last refresh"** for a genuinely in-flight multi-minute background refresh. It now reports the sweep as running, and `closedNote` is appended deliberately (a queued Twelve Data batch and a shut SGX are independent facts, and every sibling branch appends it). **The class of bug was fixed, not just the instance:** the if-chain became a `switch` with a `default` routing to `assertUnreachableOutcome(outcome: never)`, so a fifth `RefreshOutcome` value is now a **compile error** instead of another silent fall-through. **222/222** backend (215 unit + 7 integration, baseline 219 + 3) and **206/206** frontend (baseline 196 + 10), both builds clean at 0 warnings — **re-run by the orchestrating terminal, not taken from the agents' reports**. Verify-by-reverting was done on both sides; see the ⚠️ note under D36 for where that evidence is genuinely weaker than the backend's. **Not verified: nothing was driven in a browser or against a live stack** — the Z74 id 2002 free-share row was never opened in the real edit dialog, so "the UI now accepts the row the DB already holds" rests on unit tests. Open rows: **D6** only. |
| 2026-08-24 | — (orchestrator) | 11 attempted, **D36 backend closed live** | **No Phase 11 box ticked — and that is the honest outcome, not a shortfall.** Session opened to run Phase 11's four browser checks plus D36's two browser gaps. **The Chrome extension never connected** (7 attempts across `tabs_context_mcp` and `list_connected_browsers`; every structural cause ruled out from this side — extension installed and enabled `fcoeoabgfenejglbffodgkkbkcdhcgfn` v1.0.85, native messaging host registered under `HKCU\Software\Google\Chrome\NativeMessagingHosts` pointing at an existing `chrome-native-host.bat`, Chrome started well after the config was written). Docs name the remaining Windows cause as **named-pipe contention — 12 `claude` processes were running**, and its fix is restarting Claude Code, which cannot be done from inside the session. **An account-mismatch theory was raised and then disproved by the user** (Chrome's `Default` profile is a personal Google account, but the extension is signed into the work account) — recorded because the profile identity is *not* the claude.ai identity, and that is an easy wrong turn to repeat. 🛑 **The session's real find is the stale-container trap, now documented as a standing warning under Phase 11.** The stack was found already up and healthy and was taken at face value; `portfolio-api` predated its own D36 fix by 4½ hours and `portfolio-web` was **15 days old**. Had the browser connected, this session would have watched both D36 gaps fail and filed *"the D36 fix is broken."* **The extension failure was accidentally protective.** Rebuilt both images (`migrate` re-ran, exit 0), then closed **D36's backend half against the running API**: negative → 400 with the new `"cannot be negative"` string, zero → 201, Z74 qty 255→258 with cost basis unchanged at `$397.9006` and MV `$898.2325` reconciling to `258 × 3.4815211569`; probe deleted (204 → 404) and every baseline restored to the decimal. **The manual-DB-insert workaround is retired.** Also confirmed at data level (box left unticked — not seen in a browser): **Z74 converts correctly**, `4.42 SGD ÷ 1.26956` (stored rate, dated 2026-08-23) `= 3.4815211569 USD` exactly. **Credits untouched at 50/800** — the rebuild spent nothing (both exchanges closed, ledger persisted across restart, which incidentally re-confirms D39's persistence). Left undone: all four Phase 11 browser checks, both D36 browser gaps, and the blank-percentage path (needs a wholly zero-cost holding; all 25 assets are already positioned and the assets API has no DELETE, so it was left unverified rather than faked). |
| 2026-08-25 | — (orchestrator) | **11 — four browser checks closed; D36 fully closed** | **Phase 11 went from zero browser-verified boxes to five of six, and D36's last three gaps closed — all in a real browser, and all without spending a single Twelve Data credit.** 🔌 **The Chrome extension connected on the FIRST `tabs_context_mcp` probe.** No terminals were closed, `/chrome` was not run, nothing was done differently from the two sessions that failed. **The named-pipe/contention theory was already disproved on 2026-08-25 (first session); this session shows the failure is also not permanent and not reproducible on demand.** Do not spend future sessions theorising about it — probe twice, and if it works, work. **Gate A was run first and passed on evidence:** images built 2026-08-24 09:21 UTC, and `git log -3 --name-only` shows every commit since (`c663ee6`, `28e86c5`) touched **only `tracker.md` and `CLAUDE.md`** — no `src/` change was sitting undeployed, so the running stack genuinely held HEAD's application code. **The four Phase 11 checks** (full evidence in the checkboxes): fractional crypto buy `0.0123456789` ETH entered through the real form, persisting to `0.0131170789` held with cost basis `$30.8642` and recalculating the gain/loss card on screen `$2,227.83 → $2,258.69`; stock/crypto independence proven three ways including **causally** (crypto moved `2227.83 → 2258.69 → 2227.83` while stocks held at `40617.4533` to the fourth decimal); the refresh timestamp caught **running backwards** `"2 min ago" → "just now"` at 07:27:56 UTC on a document with `navigation.length === 1`, which is the only form of this evidence that isn't just a clock ticking; and Z74 read from a single in-page simultaneous fetch as `currency:"SGD"`, native `4.47`, USD `3.5181060469`, implied rate **`1.2705699999972335`** against the stored `1.27057` — with the row rendering the **converted** `$3.52`, never the SGD native. ⚠️ **D31's trap was met and handled rather than ignored:** the tab was `visibilityState:"hidden"` the whole time. That was checked *before* drawing conclusions, and `grep` established the frontend has **no** `visibilitychange` handler at all while the timestamp arrives over a push-only SignalR hub — so hidden is harmless here. A session that skipped that check would have had a result it could not defend. 🎯 **The find of the session is a correction to this file, not a code defect: D36's blank-percentage gap was never unreachable.** Both the D36 row and the ▶ Next session prompt asserted it needed a throwaway probe asset because "all 25 assets already hold priced positions". **ANVL (txn 2037) and ETH (txn 2038) were already zero-price acquisitions with `$0.00` cost basis and had been for months** — the API was already returning `unrealizedPnlPercent: null` for both, and the crypto table already rendered them with no percentage beside `+$33.84` / `+$1.93`, with AMP's `-99.14%` in the same table as a control. **No probe asset was needed or created.** The wrong claim came from reasoning over the *assets* list without querying the *transactions* that produced it — the same shape of error this file has recorded four times before under "confident arithmetic disproved by measurement". Corroborated causally: giving ETH a real cost basis flipped its percentage `null → 6.2801`, and deleting the probe returned it to `null`. **D36's other two gaps** also closed: txn 2002 opened in the real edit dialog for the first time ever (loaded `0`, zero `.ng-invalid` nodes, and the validator driven **both ways** — `-1` produced the live `"Cannot be negative."` string, back to `0` cleared it on a now-dirty control, then saved with 2002 confirmed unchanged including its notes), and a **new** zero-price transaction entered end-to-end through the form (ETH `0.005` @ `0` → units up, cost basis unchanged at `$30.8642`). 🧹 **Both probes deleted and the transaction set diffed programmatically back to byte-identical with the pre-session baseline** — 136 rows, none added, removed or changed. 💵 **Zero Twelve Data credits spent: ledger `23/800` at start, `23/800` at end.** Every check was deliberately routed through free paths (CoinGecko 2-min, Yahoo/Z74 5-min, and `/api/prices/status`, which is built never to cost a credit); the manual refresh button and backfill were left untouched **specifically so D6's in-flight measurement would not be contaminated**. 📝 Noted but deliberately **not** filed as a defect: Save stays enabled on an invalid form, but submission is genuinely guarded (clicking with `-1` left the dialog open, error shown, record untouched) — a legitimate Material pattern that keeps the error reachable rather than presenting a dead button. ⚠️ **Left undone: D6 only** — its end reading was not due during this session (NYSE opens 13:30 UTC and closes 20:00 UTC; the session ran ~07:20–07:40 UTC). Also unverified, as always: no screen reader has been near the `menuitemradio` theme toggle. |
...
Phases 1–10 and **12** are done and verified. **Phase 11 is five-sixths done** — its four browser
checks were all closed on 2026-08-25 (second session), and the container-DB box closed with Phase 10.
**D36 is fully closed, browser gaps and all.** The register is down to a single row — **D6 — and D6 is
not a defect.** It is a measurement that can only be taken while a real NYSE session is running, and
D39's closure means the credit numbers it depends on can finally be trusted.

> ⏱️ **THE ONE THING OUTSTANDING: D6's END READING. A measurement is already IN FLIGHT — do not**
> **restart it, and do not let the stack go down.** 2026-08-25 06:11:51 UTC captured the baseline
> **`daily_usage` = 24 / 800** (cross-checked against `/api/prices/status` reading 23 — divergence
> of exactly 1, which is the `/api_usage` call paying for itself, confirming D39 has not regressed).
> The stack was left up and the second session of 2026-08-25 spent **zero** further credits, so the
> baseline still stands. **What closes D6 is one reading of `GET /api_usage` after NYSE closes at
> 20:00 UTC and before the counter rolls at UTC midnight.**
>
> 🕐 **The practical window is 20:00–24:00 UTC, which is 04:00–08:00 the next morning SGT.** The
> ledger rolls the day on `DateOnly.FromDateTime(UtcNow)` (`TwelveDataCreditThrottle.cs:88`), so a
> reading taken around 23:00 UTC (07:00 SGT) is post-close, pre-reset, and does not require anyone
> to be awake at 4am. ⚠️ Twelve Data's *own* counter is **assumed** to roll at UTC midnight too —
> that has never been verified by this project, so do not cut the reading fine against 24:00 UTC.
> **The real risk to this measurement is the stack going down overnight**; that is what contaminated
> every previous attempt. Check `docker compose ps` before reading, and if the stack restarted
> mid-session say so plainly rather than reporting the number as clean.

> ⚖️ **Do NOT open with the key-rotation question.** It was asked and answered on 2026-08-24: the
> user is knowingly leaving the Twelve Data key unrotated. See the ⚖️ note under the 🔑 banner at the
> top. Treat it as settled and do not raise it again unless the user does.

> 🔌 **THE CHROME EXTENSION QUESTION IS CLOSED AS "INTERMITTENT — JUST TRY IT".** Two sessions were
> lost to it; 2026-08-25 (first session) disproved the named-pipe contention theory by measurement;
> 2026-08-25 (second session) then connected **on the first probe with nothing done differently** —
> no terminals closed, no `/chrome` run. So it is neither contention nor anything this project can
> control, and it is **not permanent**. **Standing rule, now costed three times: probe twice. If it
> connects, work. If it does not, say so early and switch to non-browser work.** Do not diagnose it,
> do not close terminals as a ritual, and do not assume a past failure predicts the next attempt.
> Two things that remain true when it *does* connect: grant site permission for `localhost:8080`,
> and prefer **element refs over screenshot coordinates** — the viewport was observed resizing
> between calls (1341×815 → 1568×675 → 1568×607), which makes coordinate clicks land in the wrong
> place, while `find` → `left_click{ref}` was reliable throughout.

> 🛑 **STILL THE FIRST INSTRUCTION: CHECK THE CONTAINERS ARE NOT STALE.** A running, healthy stack
> proves nothing about which code it holds; a `web` image was once **15 days old**. See the 🛑 block
> under Phase 11. Note the cheap shortcut 2026-08-25 used: if `git log --name-only` shows that every
> commit since the image build touched only `tracker.md`/`CLAUDE.md`, the running images still hold
> HEAD's application code and no rebuild is needed.

```
Read tracker.md. D36, D37, D38 and D39 are all CLOSED — do not redo any of them,
and do not re-ask about the API key (settled 2026-08-24, user is leaving it
unrotated). Phase 11's four browser checks are CLOSED too (2026-08-25) — do not
re-run them. THE ONLY THING LEFT IN THIS PROJECT IS D6'S END READING.

FIRST, CHECK THE STACK IS STILL UP AND STILL NOT STALE:
  docker compose ps
  docker image inspect portfolio-api --format '{{.Created}}'
  git log -1 --format=%cI
If every commit since the image build touched only tracker.md/CLAUDE.md, the
images still hold HEAD's app code — no rebuild needed. If the stack RESTARTED
since 2026-08-25 06:11 UTC, say so: the ledger persists across restarts, but a
gap in uptime means the market-hours refresh may not have run for the whole
session, and that is exactly the contamination D6 exists to avoid.

THEN TAKE THE READING — this is the whole task:
  1. Confirm NYSE has CLOSED (20:00 UTC) and the UTC day has NOT rolled (before
     00:00 UTC). Comfortable slot: ~23:00 UTC = 07:00 SGT.
  2. Read GET /api_usage's daily_usage ONCE. It costs 1 credit. Do not poll it.
  3. Baseline was 24/800 at 06:11:51 UTC on 2026-08-25. Report the full-session
     figure against 800.
  4. Cross-check against GET /api/prices/status's creditsUsedToday (free, never
     costs a credit). D39's measured residual is 3, not 200 — treat ~3 as
     healthy and anything near 200 as a D39 regression, and say which it is
     rather than trusting the ledger blindly.
  5. Tick D6's Phase 11 box ONLY with a real measured number behind it. Steady
     state projects to ~650/day at 21 symbols, but projection is exactly what
     D6 exists to replace. DO NOT tick it on arithmetic. This file has had its
     confident arithmetic disproved by measurement FIVE times now (D4/D13,
     D38's "8 req/min", D39's cost model, D6's own daily-vs-per-minute error,
     and D36's "the blank-percentage case is unreachable").

If the trading day was missed, do NOT manufacture a figure — leave D6 open and
say the window passed. It costs nothing to wait for the next trading day.

When D6 closes, Phase 11 closes and the project has no open phases. At that
point ask the user the question that has been deferred since 2026-08-24: is the
project DONE, or is there more they want built? Do not assume either.

Tick a box only for something you have personally seen pass, say plainly what
you did not verify, then append to the handoff log, write the next session
prompt, and commit.
```


**Phase 11 is now one box from closing, and that box is D6 — whose measurement is already running.**
~~Worth deciding explicitly whether the project is "done" with D6 as a permanent asterisk, or whether
someone deliberately runs a session during market hours to close it.~~ **That question is moot: the
deliberate session happened on 2026-08-25 and the baseline is captured.** D6 needs one reading after
the 20:00 UTC close, not a permanent asterisk. The question that *does* still need asking, once D6
closes and no phases remain, is whether the user considers the project **finished** or has more they
want built — see the last paragraph of the ▶ Next session prompt.
