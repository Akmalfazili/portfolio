...
**Currently active:** none — Phases 1–9 and **12** are closed out. The 2026-08-07 evening session
cleared **every open drawback except D4, D6, D7 and D13**, all four of which need something this
session could not manufacture (a maintained lunar-holiday table, a real trading day, a second
serialiser, a multi-year dataset).
...
Read the D28 row before writing compose.yaml. Nothing in src/ applies EF Core
migrations — Program.cs never touches Database, and the API image is chiseled so
it has no `dotnet ef`. A first `up` therefore starts the API against an empty
database. Pick one of the three designs in that row deliberately, and fix
README.md and this agent's own completion checklist in the same change, because
both already claim migrations are applied and neither is true today.
