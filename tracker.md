...
TWO CHEAP D36 GAPS TO CLOSE WHILE THE STACK IS UP — both are small and both are
named honestly in D36's row:
  - Open transaction id 2002 (Z74, the free-share row) in the real edit dialog
    and confirm the form now accepts the zero price it always held. The fix is
    unit-tested but has NEVER been seen in a browser.
  - Enter a NEW zero-price transaction end to end. That is the actual user
    story D36 was raised for, and the manual-DB-insert workaround should now be
    retired. Confirm the cost basis does not move and the percentage renders as
    blank/na rather than as Infinity or -100%.
