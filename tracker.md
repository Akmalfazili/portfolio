...
contaminated — see D6's row). Open rows: **D6, D36**.
...
Phases 1–10 and **12** are done and verified; only Phase 11 remains. **D39 closed 2026-08-24** — the
credit ledger now seeds and reconciles from Twelve Data's own counter, so credit figures can finally
be trusted, which is exactly the instrument **D6** needs. Open register rows: **D6, D36**.

> 🔑 **Before anything else, check whether the user has rotated the Twelve Data API key.** It was
> exposed in a transcript on 2026-08-24 (see the 🔑 banner at the top). If it has not been rotated,
> say so and stop pushing credit-spending work until it is — measuring a budget on a key that may
> need replacing wastes the measurement.

> ✅ **2026-08-24 closed D39, and how it closed is the part worth carrying forward.** The task
> mandated *measuring* the per-call cost model before changing it, and the measurement **disproved**
> the hypothesis the fix was going to be built on: `/time_series` is **not** priced by output size —
> a 5-row call and a 1,668-row call cost **1 credit each**, so the flat-1 model was already right and
> building to the theory would have skewed the ledger the other way. The real cause was structural
> (the ledger never re-checked itself against reality), now fixed by seeding and reconciling from
> `GET /api_usage`. **219/219**, live-verified, residual **3 credits instead of 200**. That is the
> third time this file's confidently-stated arithmetic has failed on contact with a measurement
> (D38's "8 req/min", D4/D13's stale estimates, now D39's cost model) — **keep measuring first.**

```
Read tracker.md, then the 🔑 key-rotation banner and the 2026-08-24 handoff entry
BEFORE anything else. D38 and D37 were fixed on 2026-08-21; D39 was fixed on
2026-08-24. Do not redo any of them. Open rows are D6 and D36.

FIRST, a gating question, not a task: has the user rotated the Twelve Data API
key? It was printed into a transcript on 2026-08-24. If not, raise it and do not
start credit-spending work until it is done.

START WITH D6 IF AND ONLY IF TODAY IS A REAL NYSE TRADING DAY AND THE STACK CAN
BE UP ACROSS THE SESSION (13:30-20:00 UTC). It is the last thing standing between
this project and "no open phases", it has been blocked since Phase 5 purely
because nobody ran the stack during market hours, and D39's closure means the
credit numbers can finally be trusted. Method:
  ...
  4. Tick D6's Phase 11 box ONLY with a real measured number behind it. Steady
     ...
     exists to replace. Do not tick it on arithmetic.
If today is NOT a trading day, skip D6 entirely rather than manufacturing a
figure, and do the frontend handoff or D36 instead.

D36 is unchanged and still available as smaller work — allow a zero PRICE (free
shares / bonus issue / scrip dividend). ValidateCommon rejects `pricePerUnit <= 0`
server-side; the Angular validator must change WITH it or the UI will reject what
the API accepts. Prefer requiring the field to be PRESENT while permitting the
value 0, so the blank-field typo guard survives. The calculators already handle a
zero cost basis (PortfolioSummaryService.cs:208), so this is validation only.

A FRONTEND handoff is outstanding and is small — `frontend-angular`, not this
agent:
  - `models.ts`'s RefreshOutcome union lacks the new 'Queued' value. Because
    describeRefreshOutcome derives its message from totalSymbolsRefreshed/sources
    (both empty for Queued), a large-portfolio manual refresh currently renders
    "Already up to date — nothing changed since the last refresh" when it in fact
    just queued a multi-minute sweep. That is actively misleading and is the one
    user-visible regression from the D38 fix.
  - `GET /api/prices/status` gained effectiveTwelveDataIntervalSeconds,
    creditsUsedToday and creditBudget. Nothing consumes them yet. Surfacing the
    interval would make refresh cadence honest as the portfolio grows, which is
    the whole point of deriving it.

Phase 11's remaining checks are still open and still reachable:
  - Enter a fractional crypto buy; confirm it persists and the gain/loss card
    recalculates.
  - /stocks and /crypto totals are fully independent.
  - Timestamp updates across a scheduled refresh with no page reload.
  - Z74 quote arrives in SGD and converts at the stored USD/SGD rate. Note Z74
    carries a zero-price transaction (id 2002, the D36 workaround), so it
    exercises a free-share holding converting from SGD as well as a normal one.

The container-DB migration-parity check is ALREADY ticked (done in Phase 10) — do
not re-verify it.

The containerized app has been LOOKED AT on only two pages (/assets, and /stocks
on 2026-08-10). If you have browser tools, confirm prices now visibly update on
screen across a refresh cycle — that has never been seen, and until 2026-08-21 it
was not even possible. Check document.visibilityState before concluding anything
visibility-sensitive; see D31.
