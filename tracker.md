...
**Currently active:** none — Phases 1–10 and **12** are closed out. Only Phase 11 remains. **D6 is
no longer blocked**: 2026-08-21 ran the stack through a real NYSE session for the first time, so a
credit measurement is now simply a matter of doing it on a trading day (that session's own number is
contaminated — see D6's row). **D36 closed 2026-08-24.** Open rows: **D6 only** — and D6 is not a
code defect, it is a measurement that needs a trading day to happen on.
...
> Left unfixed pending the user's call, but it should not be left stale.
...
Phases 1–10 and **12** are done and verified; only Phase 11 remains. **D36 closed 2026-08-24**, and
the outstanding `frontend-angular` handoff closed with it. **The register is down to a single row —
D6 — and D6 is not a defect.** It is a measurement that can only be taken while a real NYSE session
is running, and D39's closure means the credit numbers it depends on can finally be trusted.
...
> ✅ **2026-08-24 also closed the last of the messaging debt.** `describeRefreshOutcome` no longer
> tells the user "Already up to date — nothing changed" while a multi-minute background sweep is
> actually in flight. Worth carrying forward is *how* it was fixed: not just by adding the missing
> `'Queued'` branch, but by converting the if-chain to a `switch` whose `default` hits
> `assertUnreachableOutcome(outcome: never)`. A fifth `RefreshOutcome` value is now a **compile
> error**. The original bug existed purely because a union grew and nothing forced the consumer to
> notice — **when this codebase adds a value to a discriminated union, make the compiler the thing
> that catches it, not a future user report.**

> 🛑 **2026-08-24 (second session): START BY CHECKING THE CONTAINERS ARE NOT STALE.** This is now the
> first instruction in the prompt below for a reason — a session was nearly derailed by a running,
> healthy-looking stack whose `web` image was **15 days old**. See the 🛑 block under Phase 11.
>
> 🔌 **The Chrome extension did not connect for an entire session, and this is unresolved.** Seven
> attempts, every structural cause ruled out (extension installed and enabled, native messaging host
> registered correctly, Chrome restarted). The remaining documented cause is **named-pipe contention
> from other Claude Code sessions — there were 12 `claude` processes running** — and its fix,
> restarting Claude Code, cannot be performed from inside the session that needs it. **Before
> starting a browser session: close stale Claude Code terminals first, then run `/chrome` and confirm
> it reports `Status: Enabled` / `Extension: Installed`.** Do not begin the Phase 11 work until that
> panel is green; the previous session spent its whole length failing to connect. **Also grant the
> extension site permission for `localhost:8080`**, which is inherited from the extension and will
...
unrotated). The only open register row is D6.

DO THESE TWO THINGS BEFORE ANY OTHER WORK. Both cost a minute and both were
learned the hard way on 2026-08-24:
  A. VERIFY THE CONTAINERS ARE NOT STALE. A running, healthy stack proves
     nothing about which code it holds.
       docker image inspect portfolio-api --format '{{.Created}}'
       docker image inspect portfolio-web --format '{{.Created}}'
       git log -1 --format=%cI
     If either image predates HEAD, rebuild before believing ANY browser result:
       docker compose build api web && docker compose up -d
     (the `migrate` one-shot re-runs — confirm it exits 0). Last time, `web` was
     15 DAYS old and `api` predated its own D36 fix by 4.5 hours; the browser
     checks would have "failed" against code that was never deployed.
  B. CONFIRM THE CHROME EXTENSION IS ACTUALLY CONNECTED before planning browser
     work. Close stale Claude Code terminals FIRST (named-pipe contention — 12
     `claude` processes were running last time), then run `/chrome` and require
     `Status: Enabled` + `Extension: Installed`. Grant site permission for
     localhost:8080. An entire session was lost to this. Do NOT burn a session
     retrying `tabs_context_mcp`; if the panel is not green, say so early and
     switch to work that does not need a browser.

START WITH D6 IF AND ONLY IF TODAY IS A REAL NYSE TRADING DAY AND THE STACK CAN
BE UP ACROSS THE SESSION (13:30-20:00 UTC). It is the single thing standing
between this project and "no open phases". Method:
  1. Bring the stack up BEFORE 13:30 UTC and leave it up through 20:00 UTC.
  2. Read `GET /api_usage`'s `daily_usage` at the start and at the end. Each read
     costs 1 credit — budget for that, and do NOT poll it in a loop.
  3. Report the full-session figure against 800, and cross-check it against
     `GET /api/prices/status`'s `creditsUsedToday`, which should now track it
     closely (D39). A large divergence means D39 regressed — say so rather than
     trusting the ledger. D39's measured residual was 3 credits, not 200; treat
     anything in that neighbourhood as healthy and anything near 200 as a
     regression.
  4. Tick D6's Phase 11 box ONLY with a real measured number behind it. Steady
     state projects to ~650/day at 21 symbols, but projection is exactly what D6
     exists to replace. Do not tick it on arithmetic. This file has now had its
     confident arithmetic disproved by measurement FOUR times (D4/D13, D38's
     "8 req/min", D39's cost model, and D6's own daily-vs-per-minute error).
If today is NOT a trading day, skip D6 entirely rather than manufacturing a
figure, and do the Phase 11 browser checks instead.

Phase 11's four reachable checks are all that is left besides D6, and every one
of them wants a BROWSER, not a unit test:
...
D36'S REMAINING GAPS ARE NOW BROWSER-ONLY. The backend half was closed against
the RUNNING API on 2026-08-24 (negative -> 400 "cannot be negative", zero -> 201,
Z74 qty 255->258 with cost basis unchanged at $397.9006, probe deleted and
baseline restored). Do NOT re-run the API probes. What is left:
  - Open transaction id 2002 (Z74, the free-share row) in the real edit dialog
    and confirm the form now accepts the zero price it always held. Never once
    seen in a browser.
  - Enter a NEW zero-price transaction through the FORM, end to end. The API
    path is proven; the form path is not. The manual-DB-insert workaround is
    already retired.
  - The blank/na percentage (rather than Infinity or -100%) is STILL UNVERIFIED
    and is NOT reachable by adding free shares to an existing position — it
    needs a holding whose ENTIRE cost basis is zero. All 25 assets already hold
    priced positions and the assets API has no DELETE, so a probe asset would
    permanently pollute the portfolio. It was deliberately left unverified
    rather than faked. If you want it closed, decide with the user first whether
    a throwaway asset is acceptable and how it would be removed.

The container-DB migration-parity check is ALREADY ticked (Phase 10) — do not
re-verify it.

The containerized app has been LOOKED AT on only two pages (/assets, /stocks).
Prices have never been SEEN updating on screen. Check document.visibilityState
before concluding anything visibility-sensitive; see D31.
