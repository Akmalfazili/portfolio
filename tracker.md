...
entries. D6 needs a real NYSE trading day, which no local session can manufacture — Phase 11 is
otherwise the only thing left, and four of its six checks are already reachable today.

> 🐛 **2026-08-10 changed the register — do not trust an older summary saying "only D6 is open".**
> **D35** (the FX backfill starved of its one provider call, 500ing the entire `/stocks` page) was
> found and **fixed** the same day, verified on the live Docker stack and in a browser. It left two
> new open rows behind: **D36** — a zero-price acquisition (free shares, bonus issue, scrip
> dividend) cannot be entered at all, because `TransactionService.ValidateCommon` rejects
> `pricePerUnit <= 0` **server-side**; the user's 5 free Z74 shares are in the database via a direct
> `INSERT` (transaction id 2002), so **any future free-share record needs the same manual step until
> this is fixed**. And **D37** — no rate-limit pacing, so one backfill run structurally cannot cover
> every asset (HLAL and JNJ still 429'd post-fix). D37 is benign and self-correcting across runs;
> D36 is a real functional gap with a hand-worked-around data record behind it. **Open rows: D6,
> D36, D37.** Neither new row blocks Phase 11.
...
Note the Z74 check above is now more interesting than when it was written: that
asset carries a zero-price transaction (id 2002, the D36 workaround), so it
exercises a free-share holding converting from SGD as well as a normal one.

Two NEW rows are also available if Phase 11 is blocked or you want smaller work:
  - D36 — allow a zero PRICE (free shares / bonus issue / scrip dividend).
    ValidateCommon rejects `pricePerUnit <= 0` server-side; the Angular validator
    must change WITH it or the UI will reject what the API accepts. Decide one
    thing deliberately: allowing 0 removes the blank-field typo guard, so prefer
    requiring the field to be PRESENT while permitting the value 0. Note the
    calculators already handle zero cost basis (PortfolioSummaryService.cs:208),
    so this is a validation change only.
  - D37 — rate-limit pacing for Twelve Data (8 req/min). Keep it off the request
    path; a paced full run takes minutes.
