...
describe('describeValidationError', () => {
  it('renders the shared codes with the wording the dialogs used before it moved', () => {
    ...
    expect(describeValidationError({ positive: true }, 'Invalid value.')).toBe('Must be greater than zero.');
    expect(describeValidationError({ negative: true }, 'Invalid value.')).toBe('Cannot be negative.');
    expect(describeValidationError({ min: { min: 0, actual: -1 } }, 'Invalid value.')).toBe('Cannot be negative.');
    expect(describeValidationError({ max: { max: 12, actual: 13 } }, 'Invalid value.')).toBe('Must be 12 or less.');
  ...
  it('gives required precedence over the value-shape codes — a blank field is not "too precise"', () => {
    expect(describeValidationError({ required: true, maxSignificantDigits: { max: 28 } }, 'Invalid value.')).toBe(
      'Required.',
    );
  });
  ...
  it('uses the caller-supplied fallback for an unrecognised code', () => {
    expect(describeValidationError({ matDatepickerParse: true }, 'Invalid date.')).toBe('Invalid date.');
  ...
  it('falls through an empty-string server message rather than rendering blank text', () => {
    ...
    expect(describeValidationError({ server: '', required: true }, 'Invalid value.')).toBe('Required.');
  });

  it('has no shared wording for pattern — it falls through unless the caller supplies one', () => {
    expect(describeValidationError({ pattern: { requiredPattern: '^[A-Za-z]{3}$' } }, 'Invalid value.')).toBe(
      'Invalid value.',
    );
    expect(
  ...
  it('lets a caller reword a shared code in its shared precedence position (asset-form.dialog.ts min)', () => {
    ...
    expect(describeValidationError({ min: { min: 1, actual: 0 } }, 'Invalid value.', rangeMin)).toBe(
      'Must be 1 or greater.',
    );
    // Still outranked by server and required, exactly as before.
    expect(describeValidationError({ min: { min: 1 }, required: true }, 'Invalid value.', rangeMin)).toBe('Required.');
    expect(describeValidationError({ min: { min: 1 }, server: 'Nope.' }, 'Invalid value.', rangeMin)).toBe('Nope.');
  });
});
